...
2. The deterministic baseline selected 0 of 30 evaluation-slice candidates. Stage 8 (`docs/stage-8-quant-economic-diagnostic.md`) traced the cause; Stage 9 (`docs/stage-9-barrier-model-experiment.md`) then ran a walk-forward barrier experiment with proven label parity. Result: a trained LightGBM model **does** beat the trailing-frequency baseline out of sample (AUC 0.525→0.535, calibrated), but the information is uneconomic — `required_p_target > 1.0` in every fold, and the maximum achievable gross (2.4–11.1 bps) is below the 15 bps round-trip cost at **every** measured geometry. The binding constraint is barrier geometry vs transaction cost, not model quality. Stage 10 A/B must not open until a versioned economics experiment establishes a positive-expectancy geometry.
...
Stages 4–9 are documented in `docs/stage-4-7-report.md`, `docs/stage-8-quant-economic-diagnostic.md`, and `docs/stage-9-barrier-model-experiment.md`. Stage 4 (provider reliability) and Stage 5 (deterministic recovery) **PASS**. Stage 7 and Stage 10 A/B remain **not executed**.
